...
args.add_argument("--num_filters", type = int,default = 100) # "Number of filters per filter size"
args.add_argument("--dropout_rate", type = float, default = 0.5) # "Dropout keep probability"
args.add_argument("--l2_reg_lambda", type = float, default = 3) # "L2 regularization lambda"

# Training parameters
args.add_argument("--batch_size", type=int, default=50) #Mini-batch size used for training
args.add_argument("--num_epochs", type = int, default = 10) # "Number of training epochs"
...
x_dev, y_dev = reshape_data(x_shuffle[1500:2000].reset_index(drop=True), y_shuffle[1500:2000].reset_index(drop=True))
...
with tf.Graph().as_default():
    ...
    for i in range(FLAGS.num_epochs//FLAGS.evaluate_every):
        ...
        l,g = cnn.evaluate(x_dev,y_dev,batch_size = FLAGS.batch_size)
